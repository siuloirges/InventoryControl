<?php namespace App\Http\Controllers;
	...
	class AdminRecursosController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Asunto","name"=>"Asunto"];
            ...
            $Informacion = Recursos::INFORMACION_TYPE;
            $Pregunta = Recursos::PREGUNTA_TYPE;
            $this->form = [];
			$this->form[] = ['label'=>'Tipo de recurso','name'=>'type','type'=>'select2','validation'=>'required|min:1|max:255','width'=>'col-sm-10','dataenum'=>"$Informacion;$Pregunta"];
			$this->form[] = ['label'=>'Asunto','name'=>'Asunto','type'=>'textarea','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Descripción','name'=>'Descripcion','type'=>'textarea','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Foto','name'=>'picture','type'=>'upload','validation'=>'image|max:3000','width'=>'col-sm-10','help'=>'Tipo de imágenes soportados: JPG, JPEG, PNG, GIF, BMP'];
			$this->form[] = ['label'=>'Archivo','name'=>'file','type'=>'upload','width'=>'col-sm-10'];

			if(CRUDBooster::isSuperadmin()){
                $this->form[] = ['label' => 'Producto', 'name' => 'products_id', 'type' => 'select2', 'validation' => 'integer|min:0', 'width' => 'col-sm-10', 'datatable' => 'products,name','datatable_format'=>"name,' - ',type"];
                ...
                $this->form[] = ['label' => 'Producto', 'name' => 'products_id', 'type' => 'select2', 'validation' => 'integer|min:0', 'width' => 'col-sm-10', 'datatable' => 'products,name','datatable_format'=>"name,' - ',type","datatable_where"=>"stores_id = $store_id and type = 'PRODUCT'"];
...
//             $this->form[] = ['label' => 'Productos', 'name' => 'products_id', 'type' => 'select2', 'validation' => 'required|integer|min:0', 'width' => 'col-sm-10', 'datatable' => 'products,name',"datatable_where"=>"stores_id = $store_id and type = 'PRODUCT'",'datatable_format'=>"name,' - ',type"];
			...
			//$this->form[] = ["label"=>"Asunto","name"=>"Asunto","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
			//$this->form[] = ["label"=>"Descripción","name"=>"Descripción","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
