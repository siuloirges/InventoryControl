<?php namespace App\Http\Controllers;
	...
	class AdminCustomers1Controller extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Apellido","name"=>"last_name"];
			$this->col[] = ["label"=>"Direccion","name"=>"address"];
			...
			$this->col[] = ["label"=>"Teléfono 2","name"=>"phone_other"];
			$this->col[] = ["label"=>"Correo Electronico","name"=>"email"];
            $this->col[] = ["label"=>"Tipo de documento","name"=>"type_document_id","join"=>"type_documents,name"];
            ...
            if ( $id_cms_privileges != "2" ) {
                ...
                $this->form[] = [ 'label'=>'Asesor', 'name'=>'user_id', 'type'=>'text', 'value' => CRUDBooster::myId(),'readonly' => 'true'];
            }
			$this->form[] = ['label'=>'Nombre','name'=>'name','type'=>'text','validation'=>'required|string|min:3|max:70','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Apellido','name'=>'last_name','type'=>'text','validation'=>'required|string|min:3|max:70','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Dirección','name'=>'address','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			...
			$this->form[] = ['label'=>'Teléfono','name'=>'phone','type'=>'number','validation'=>'required|numeric','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Teléfono 2','name'=>'phone_other','type'=>'text','validation'=>'','width'=>'col-sm-10'];
			$this->form[] = ['label'=>'Correo Electronico','name'=>'email','type'=>'text','validation'=>'string','width'=>'col-sm-10'];
			...
			$this->form[] = ['label'=>'Descripcion','name'=>'description','type'=>'textarea','validation'=>'','width'=>'col-sm-10'];
...
//            if (!CRUDBooster::isSuperadmin()) {
                $store_id = DB::table('cms_users')->where('id', '=',CRUDBooster::myId() )->first()->stores_id;
                $query->where('customers.stores_id',  $store_id);
            }

//            if (!CRUDBooster::isSuperadmin() && $id_cms_privileges!="3" ) {
//                $query->where('customers.user_id',  CRUDBooster::myId());
//            }
